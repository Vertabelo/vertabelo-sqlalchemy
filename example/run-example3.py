...
import example_model3
from example_model3 import Product
from example_model3 import ProductCategory
from example_model3 import OrderItem
...
def show():
    ...
    for order_item in session.query(OrderItem).filter(OrderItem.order_id == current_order_id):
        print("%-5s - %-20s - %-5s - $%s" % (order_item.product.sku, order_item.product.name, order_item.amount, order_item.amount * order_item.product.price))
